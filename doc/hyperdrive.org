...
Please be sure to [[*Install hyper-gateway-ushin][install hyper-gateway-ushin]].
...
- User Option: hyperdrive-gateway-process-type ::
  ...
  have configured [[https://git.sr.ht/~ushin/hyper-gateway-ushin#how-do-i-run-hyper-gateway-ushin-as-a-background-process-on-gnulinux--systemd][hyper-gateway-ushin to run as a systemd service]],
...
For full usage instructions, see the [[https://git.sr.ht/~ushin/hyper-gateway-ushin#usage][hyper-gateway-ushin README]].
...
hyperdrive (see [[*Versioning][the prior section]]), my gateway would also load the
start of the version range containing the most recent version of
~/bar.org~.  My ~hyperdrive.el~ knows the latest version number of your
drive, but with partial version history, the history of ~/bar.org~ in
...
If I run ~M-x hyperdrive-previous~ while looking at the latest version
of ~/bar.org~, ~hyperdrive.el~ will display ~/bar.org~ at version 52.  In
the background, the gateway will report that ~/bar.org~ at version 52 of
...
- User Option: hyperdrive-hyper-gateway-ushin-port ::
  ...
  ~hyperdrive.el~ will send HTTP requests to ~hyper-gateway-ushin~ on this port.
  Defaults to ~4973~.
...
# TODO: Update the next paragraph to mention install/upgrade command and omit version numbers.
