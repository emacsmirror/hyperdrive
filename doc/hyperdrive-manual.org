...
#+kindex: hyperdrive-dir-copy-url-as-kill
...
- Public key :: public, globally unique, not human-readable
- Petname    :: private, locally unique, human-readable
- Nickname   :: public, not necessarily unique, human-readable

Each drive may also have the following two attributes:

- Seed       :: string used to generate public key
- DNS domain :: public, globally unique, human-readable
...
from your secret master key and a seed string. The secret key is
generated for you by ~hyper-gateway~, and you provide a seed ([[*Seeds]])
when generating a new drive with ~hyperdrive-new~.
