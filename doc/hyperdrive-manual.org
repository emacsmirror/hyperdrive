...
Once ~hyperdrive.el~ has loaded, the "Hyperdrive" menu should appear at
the top of the screen, offering an overview of the available commands.
If you don't see the "Hyperdrive" menu, please double check that
~menu-bar-mode~ is enabled (it is enabled by default).

** hyperdrive-menu
#+findex: hyperdrive-menu

~M-x hyperdrive-menu~ offers quick access to many ~hyperdrive.el~
...
commands are available in ~hyperdrive-menu~ when you're inside a
