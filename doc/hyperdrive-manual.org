...
Relative links are supported, so within ~hyper://PUBLIC-KEY/foo.org~,
links to ~bar.org~, ~./bar.org~, and ~/bar.org~ will all point to
~hyper://PUBLIC-KEY/bar.org~. A version number can also be specified in
relative links, so that ~/$/version/42/bar.org~ will point to
~hyper://PUBLIC-KEY/$/version/42/bar.org~.

** View the hyperdrive version history
#+findex: hyperdrive-previous-version

Hyperdrives are versioned ([[*Versioning]]).  To view the previous version
of a hyperdrive file, run ~hyperdrive-previous-version~ inside the
file's buffer.
...
For an explanation of the history buffer, [[*Partial version data]].
...
command will first prompt for confirmation.

*Remember that data which has been purged from your local machine may
still be available on the network.*
...
Because hyperdrives are sparsely replicated ([[*Sparse replication]]), you
...
([[*Hyper-gateway]]) will also return the start of the version range
