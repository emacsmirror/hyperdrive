...
- ~hyperdrive-reuse-buffers~ :: How to reuse buffers when showing entries.
  When ~any-version~, try to reuse an existing buffer showing
  the same entry at any version.  When ~same-version~, try to reuse an
  existing buffer at the same version, or make a new buffer.
