...
| Status                                   | Command                 | Correct behavior? |
|------------------------------------------+-------------------------+-------------------|
| buffer is backed by writable hyperdrive  | hyperdrive-write-buffer |                   |
| buffer is backed by read-only hyperdrive | hyperdrive-write-buffer |                   |
| buffer is backed by file on disk         | hyperdrive-write-buffer |                   |
| buffer is not backed by anything         | hyperdrive-write-buffer |                   |
| buffer is backed by writable hyperdrive  | hyperdrive-save-buffer  |                   |
| buffer is backed by read-only hyperdrive | hyperdrive-save-buffer  |                   |
| buffer is backed by file on disk         | hyperdrive-save-buffer  |                   |
| buffer is not backed by anything         | hyperdrive-save-buffer  |                   |
