...
Therefore, we should treat them the same way: as both pointing to files on the same filesystem as the document in which they are written, i.e. ~file:~ and protocol-prefix-less links within a hyperdrive file should be treated as links to files within that hyperdrive--never as links to files on the local filesystem.  (This is akin to connecting to a remote system via TRAMP and viewing Org files that way.)
